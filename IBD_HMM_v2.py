#fixed so that major major probabilities are incorporated
...
def data_prep(data_df):
    ...
    data_df.insert(7,'d_IBD', np.log(2*eps * (1-eps)))
    d_DBD = 2*eps*(1-eps)*(data_df['minor_prop']**2 + (data_df['major_prop'])**2) + 2*(1-eps)**2*data_df['minor_prop']*data_df['major_prop'] + 2*eps**2*data_df['minor_prop']*data_df['major_prop']
    data_df.insert(8,'d_DBD', np.log(d_DBD))
    
    #concordance condition
    c_IBD = (1-eps)**2*data_df['minor_prop'] + eps**2*data_df['major_prop'] + (1-eps)**2*(data_df['major_prop']) + eps**2*data_df['minor_prop']
    data_df.insert(9,'c_IBD', np.log(c_IBD))
    c_DBD =(1-eps)**2*data_df['minor_prop']**2 + 2*eps*(1-eps)*data_df['minor_prop']*data_df['major_prop'] + eps**2*data_df['major_prop']**2 + (1-eps)**2*(data_df['major_prop'])**2 + 2*eps*(1-eps)*data_df['minor_prop']*data_df['major_prop'] + eps**2*data_df['minor_prop']**2
    data_df.insert(10,'c_DBD',np.log(c_DBD))
    ...
    for sample in data_df.keys()[7:]:
        super_ref_df[sample] = data_df[sample] == data_df['ref']
...
def pairwise_df(good_df):
    fout = open(base+'_results_v2.txt', 'w')
...
def viterbi(df):
    ...
    for number in range(1,17):
        ...
        if path != 'None':
            path = path[::-1]
        #unreverse to map back to chromosome
...
if __name__ == "__main__":
    ...
    base = ('_').join(good_combos_file.split('_')[0:-2])
